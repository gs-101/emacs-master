...
This gets some information to define the package.
...
The longest part of this is the =git clone=. It just takes too long to clone Emacs, but I don't think it can be optimized. I tried to make the it shallow, but that unfortunately generates a different hash.

~commit~ is used in the next step to check if the package should actually be updated.
