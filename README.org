...
Savannah repositories seem to be currently be inaccessible, so this channel doesn't work for now! I'll submit this to [[https://toys.whereis.social/][Toys]] once it's ready.
...
#+begin_src yaml
      - name: Compare Commits
        id: compare-commits
        run: |
          if [ "$new-commit" != "$current-commit" ]; then
